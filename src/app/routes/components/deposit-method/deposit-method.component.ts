import { Component, Injector, OnInit, ViewChild, Output, EventEmitter, ElementRef, HostListener } from '@angular/core';
...
@Component({
  ...
  styleUrls: ['./deposit-method.component.less']
})
export class DepositMethodComponent implements OnInit {
    ...
    this.http.getAllPSP('deposit').subscribe((res:any)=> {
       ...
       this.pspList = content
        ...
        if(tabName === 'BankTransfer') {
          if(this.userNationalDeposit === 'AU' && this.userStateDeposit === null  ||  this.userNationalDeposit === 'AU' && this.userStateDeposit === "" ) {
  ...
  submitForm(): void {
    ...
    if (this.validateForm.valid) {
      const { wallet, walletNumber, accounts, currencie, banklist, hyperBCCoin, paymentreferID, country, state,city, street, zipCode, pesronalId, walletID} = this.validateForm.value;
    ...
    //  console.log(TrimAmount)
      ...
      if (amount < 20) {
        this.message.error('Minimum Amount Should be 20 USD');
        return;
      }
       if (pspName === 'BankTransfer') {
         this.router.navigate([`${this.langs}/payment/alternative`], {
           queryParams: {
             customer_ID: this.tokenSrv.get()?.customer_id,
             deposiT_To: 1,
             paymenT_DESTINATION: sessionStorage.getItem('wallet'),
        ...
        const currencyValue =
          this.validateForm.get('wallet')?.value == 1 ? this.walletBalance.currency : this.validateForm.value.accounts.split('|')[1];
        const params = {
          customer_ID: this.tokenSrv.get()?.customer_id,
          deposiT_To: 1,
          paymenT_DESTINATION: sessionStorage.getItem('wallet'),
        ...
        let body = {
         ...
         this.http.depositEpay(body).subscribe((res:any)=> {
          let data = res.data
            const options = {
                ...
                orderCurrency: data.orderCurrency,
                 ...
                 countrycode:data?.countrycode,
                  ...
                  failedHandler:async (res:any) => {
                    ...
                    this.loaderSubmit = false
                    this.http.epayCallBack(res, data.callSign).subscribe((res:any)=> {
                    })
                    console.log(res)
                  },
        ...
        this.http.depositUPI(body).subscribe((res:any)=> {
          let data = res.data
        ...
        //   queryParams: {
        //     deposiT_To: wallet,
        //     paymenT_DESTINATION: this.validateForm.get('wallet')?.value == 1 ? walletNumber : sessionStorage.getItem('wallet'),
        //     deposiT_AMOUNT: amount,
        //     deposiT_TYPE: '2',
        //     currency: currencie.split('|')[1],
        //   }
        ...
        this.http.depositHelppay(body).subscribe((res:any)=> {
          let data = res.data
          if (res.statusCode == 101) {
        ...
        error => {
         ...
         this.http.depositHyperBC(body).subscribe((res:any)=> {
             let data = res.data
             if (res.statusCode == 101) {
         ...
         this.http.depositLean(body).subscribe((res:any)=> {
             let data = res.data
             if (res.statusCode == 101) {
        ...
        this.http.gpDeposit(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
            this.loaderSubmit = false
            let content = res.data
        ...
        error => {
         ...
         this.http.paDeposit(body).subscribe((res:any)=> {
           this.loaderSubmit = false
           let content = res.data
           $(document).ready(function(){
             $(content).appendTo('body').submit();
        ...
        this.http.skrillDeposit(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
            this.loaderSubmit = false
            let content = res.data
            $(document).ready(function(){
      ...
      // }
        ...
        this.http.bvnkDeposit(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
            this.loaderSubmit = false
        ...
        this.http.bridgerpayDeposit(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
            this.loaderSubmit = false
             window.open(res?.data, '_self');
          //  this.isVisibleBVNK = true
        ...
        this.http.blizzardpayDeposit(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
            this.loaderSubmit = false
             window.open(res?.data, '_self');
          //  this.isVisibleBVNK = true
        ...
        this.http.egPayDeposit(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
            this.loaderSubmit = false
            if(this.egWalletPay) {
              this.message.success(this.i18nSev.i18n('Your Deposit has been Initiated Successfully, Kindly Transfer to this Vodafone Wallet from your Personal Number'));
            } else {
              this.message.success(this.i18nSev.i18n('Your Deposit has been Initiated Successfully, We will Contact you Shortly'));
            }
            this.router.navigateByUrl(`${this.langs}/funds/wallet`);
          }
        },
        ...
        this.http.exlinkDeposit(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
            this.loaderSubmit = false
            window.open(res?.data, '_self');
          }
        ...
        this.http.cheezePay(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
            this.isVisibleCheeze = true
            this.loaderSubmit = false
            this.cheezeUrl = this.sanitizer.bypassSecurityTrustResourceUrl(res?.data?.url); 
        ...
        this.http.ccoopDeposit(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
           this.isVisibleCop = true
            this.loaderSubmit = false
            this.copUrl = this.sanitizer.bypassSecurityTrustResourceUrl(res?.data?.redirectUrl); 
        ...
        this.http.oneTwopay(body).subscribe((res:any)=> {
          if(res.statusCode === 101)  {
           this.isVisibleOneTwo = true
            this.loaderSubmit = false
            this.oneTwoUrl = this.sanitizer.bypassSecurityTrustResourceUrl(res?.data?.code_image); 
  ...
  getExchangeRate(FromCurrency: string, ToCurrency: string) {
    ...
    this.http.getExchangeRate({ FromCurrency, ToCurrency, TransactionType: 'deposit', PaymentMethod: this.egpayHide ? 29 : 0  }).subscribe((res: any) => {
    ...
    const { accounts, amount } = this.validateForm.value;
     if (this.cryptoForm.valid) {
      ...
      //  })
        ...
        this.http.coinDeposit(params).subscribe((res:any) => {
          let data  = res.data
          if (res.statusCode == 101) {
  ...
  showFatoorahModal(param:any):void {
    ...
    console.log(param)
    this.http.getAvailablePaymentMethods(param).subscribe((res:any)=> {
      if(res.statusCode === 101) {
  ...
  showHyperBCModal(param: any): void {
    ...
    this.hyperCoinSelected = this.hyperBCCoinList[this.validateForm.controls['hyperBCCoin'].value];
  ...
  getDepositStatusByID(param: any): void {
    console.log(param);
    this.http.GetDepositById(param).subscribe((res:any)=> {
        let data = res.data;
        console.log(data);
    ...
    let body = {
      ...
      callSign: this.callSign,
       ...
       paymentMethodData: {
         ...
         cardNetwork: event?.detail?.paymentMethodData?.description?.info?.cardNetwork,
         cardDetails: event?.detail?.paymentMethodData?.description?.info?.cardDetails
       }
    }
  ...
  submitFatoorah() {
    ...
    this.http.depositFatoora({...this.params, paymenT_METHOD_ID: this.accountDetails.paymentMethodId }).subscribe((res:any)=> {
      window.open(res.data, '_self');
  ...
  validateAlterAus() {
    ...
    this.validateForm.controls['state']?.setValidators([Validators.required, Validators.pattern("^.{1,3}$")]);
    this.validateForm.controls['city']?.setValidators([Validators.required]);
    this.validateForm.controls['street']?.setValidators([Validators.required, Validators.pattern("^.{5,100}$")]);
  ...
  tableClick(tab:any) {
    console.log(tab.availablePSPs[0]?.name + '|' + tab.availablePSPs[0]?.default_Currency)
