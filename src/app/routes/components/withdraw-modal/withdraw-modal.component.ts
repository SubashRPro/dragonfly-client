...
export class WithdrawModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.validateForm = this.fb.group({
      ...
      coinName: [null],
      chainName: ["TRC20"],
      toAddress: [{ value: this.addressValue, disabled: true }],
  ...
  submitForm(): void {
    ...
    if (this.validateForm.valid) {
      ...
      if (pspName === "crypto") {
        ...
        ["bankTransfer", "creditDebitCards", "eWallets"].includes(pspName)
      ) {
        let body = {
          paymenT_SOURCE: accounts.split("|")[0],
          withdraW_AMOUNT: amount,
        ...
        this.message.error("No valid PSP type found");
      }
    } else {
    ...
    this.loadingdata = true;
     // ✅ File validation
      if (!this.fileList || this.fileList.length === 0) {
        this.message.error('Please upload a QR code image in JPEG, JPG, or PNG format to proceed.');
        this.loadingdata = false;
        return;
      }
    const files =  await this.onFileToBase64();
    if (this.walletForm.valid) {
      const { wallet_Address , networkName } = this.walletForm.value;
      let body = {
        wallet_Address: wallet_Address,
        network_Channel:networkName
      } 

      this.http.addWallet({...body, qrCode_Data: { ...files[0] } }).subscribe(
        (res:any) => {
  ...
  getAllWallets() {
    this.http.getAllCryptoWallets().subscribe((res: any) => {
      this.wallets = res.data;
    });
  }

selectedWallet: any = null;
coinSelection:boolean = false;
addressValue:any
selectWalletAccount(selectedRow: any) {
  this.validateForm.patchValue({
    chainName: selectedRow.network_Channel,
    toAddress: selectedRow.wallet_Address
  });

  this.addressValue = selectedRow.wallet_Address;
  this.isVisibleBVNK = false; // close modal
  this.coinSelection = true;
}
