...
@Component({
  selector: 'app-tripea',
  templateUrl: './triplea.component.html',
  styles: []
})
export class TripeAComponent implements OnInit {
  kycMessage!:boolean
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(param => {
      this.param = { ...param };
      this.http.getDepositByCardtripeAPaymentAsync({ ...this.param, postType: undefined }).subscribe((res:any)=> {
        const result = res?.data;
        this.ifromUrl = this.sanitizer.bypassSecurityTrustResourceUrl(result.hostUrl);
        this.loading = false;
      },
      error => {
        this.loading = false;
