...
import { GetCodeParams , GetTokenParams} from 'src/app/models/users';
...
export class SetPasswordComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.subscribe(queryParams => {
      let uri = queryParams?.sptkn
      this.setId = encodeURIComponent(uri)
    });
  ...
  submitForm(): void {
    ...
    if(this.proceedStepNo == 3) {
      if(this.registerForm.valid) {
        ...
        let formData = {
          otp: this.emailOtp,
          setPasswordToken: this.setId,
          new_Password: this.registerForm.value.password,
          is_AgreedOnTC: this.registerForm.value.agree
        };
        this.api.setPassword(formData).subscribe(
          (res: any) => {
            this.isVisible = true
            this.currentStep = 4;
            this.topStep4 = true;
      ...
      this.api.validateToken({
        ...
        .subscribe( (res: any) => {
            if(res.data) {
              ...
              this.showEmail = res.data.mEmail
              this.currentStep = 2;
              this.topStep2 = true;
              this.loader = false;
            }
