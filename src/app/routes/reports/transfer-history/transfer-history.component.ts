...
export class TransferHistoryComponent implements OnInit {
  ...
  public getReport(pageNumber: any, itemsPerPage: any, sortBy:any, sortOrder:any) {
    ...
    this.loading = true;
    this.common.getAllTransfersReport(this.filter).subscribe(
      (res: any) => {
  ...
  exportAsXLSX(): void {
    // this.excelService.exportAsExcelFile(this.users, 'Transfer-history');
    let csvStr = `${this.i18NService.i18n('Bill No')},${this.i18NService.i18n('Created Time')},${this.i18NService.i18n('From Account')} ,${this.i18NService.i18n(
      'To Account'
    )},${this.i18NService.i18n('Amount')},${this.i18NService.i18n('Currency')},${this.i18NService.i18n('Status')} \n`;
    for (let data of this.users) {
      var recordStr = `${data.transaction_ID},${data.transfeR_TIME},${data.paymenT_SOURCE},${data.paymenT_DESTINATION},${data.price},${data.to_Currency},${data.status}`;
    ...
    saveAs(csvBlob, 'Transfer history.csv');
  }
}
